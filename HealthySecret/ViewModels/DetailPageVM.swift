...
class DetailPageVM : ViewModel {
    ...
    func transform(input: Input, disposeBag: DisposeBag ) -> Output {
        ...
        input.addBtnTapped.subscribe(onNext: { _ in
            ...
            let exercise = Exercise(date: UserDefaults.standard.string(forKey: "date") ?? "", name: name , time: time  , finalCalorie : finalCalorie , memo : memo )

            self.firebaseService.updateExercise(exercise: [exercise], key: UserDefaults.standard.value(forKey: "email") as! String).subscribe{ com in
                switch com {
                case .completed:
